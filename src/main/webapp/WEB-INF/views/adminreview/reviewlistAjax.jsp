...
<div class="pagination">
    ...
    <ul>
        <c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
            <li class="${num == paging.currentPage ? 'current' : ''}">
                <a href="${path}/review.ad?page=${num}" class="btn page-link" data-page="${num}">${num}</a>
            </li>
        </c:forEach>
    </ul>
    ...
    <c:if test="${paging.endPage < paging.pageCount}">
        <a href="${path}/review.ad?page=${paging.next}" class="btn next page-link" data-page="${paging.next}">
            <svg xmlns="http://www.w3.org/2000/svg" class="svg">
                <path d="m2 6 6-4.33v8.66L2 6z" />
            </svg>
        </a>
